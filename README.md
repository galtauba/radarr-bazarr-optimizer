...
עדיפות מקור הגדרות:
- מקור האמת בריצה הוא `SQLite` (`settings` בטבלת DB).
- קובץ `.env` משמש בעיקר לערכי seed/ברירת מחדל.
- אם ה־DB כבר מאותחל, שינוי ב־`.env` לא דורס אוטומטית הגדרות קיימות ב־DB.
- במסך `Onboarding`, שדות נטענים מערכי `.env`; אם משתנה לא מוגדר, השדה יוצג ריק.
...
- המזהה הראשי במערכת הוא `radarr_movie_id`.
- אם סרט נמחק מ־Radarr ונוסף מחדש עם `radarr_movie_id` חדש, המערכת מנסה לבצע relink לפי `imdbId`.
- relink יתבצע רק אם נמצא התאמה חד-משמעית אחת לסרט שהוסר בעבר עם אותו `imdbId`.
- אם אין `imdbId` או שיש יותר מהתאמה אחת (עמימות), relink לא מתבצע והסרט יטופל כסרט חדש.
